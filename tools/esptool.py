...
def read_mac(esp, args):
    mac = esp.read_mac()
    print '%s' % ''.join(map(lambda x: '%02X' % x, mac))

def gen_key(esp, args):
    mac = esp.read_mac()
    print hashlib.sha1('%scdaqk2325kcl' % ':'.join(map(lambda x: '%02x' % x, mac)).upper()).hexdigest() 

def chip_id(esp, args):
    chipid = esp.chip_id()
    print 'Chip ID: %d' % chipid
